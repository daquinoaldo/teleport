...
func NewHTTPClient(cfg client.Config, tls *tls.Config, params ...roundtrip.ClientParam) (*HTTPClient, error) {
	...
	if cfg.EnableCircuitBreaker {
		breakerConfig := breaker.Config{
			Clock:         cfg.Clock,
			Interval:      apidefaults.BreakerInterval,
			RecoveryLimit: apidefaults.RecoveryLimit,
			Trip:          breaker.RatioTripper(apidefaults.BreakerRatio, apidefaults.BreakerRatioMinExecutions),
			IsSuccessful:  client.IsHTTPResponseSuccessful,
		}
